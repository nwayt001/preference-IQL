...
def get_all_examples_from_gym_env(relative_dataset_addr):
    mp4_files = sorted([os.path.join(relative_dataset_addr, f) for f in os.listdir(relative_dataset_addr) if f.endswith(".mp4")])
    return mp4_files
...
def parse_args():
    ...
    parser.add_argument("--minerl_data_root", type=str, default=f"{os.getenv('HOME')}/workspace/kairos_minerl_22/data")
    parser.add_argument("--basalt_gym_env", type=str, default="MineRLBasaltFindCave-v0")
    parser.add_argument("--use_yolo_labels", type=bool, default=False)

    args = parser.parse_args()
    args.relative_dataset_addr = os.path.join(args.minerl_data_root, args.basalt_gym_env)
    return args
...
def main():
    ...
    dataset_examples = get_all_examples_from_gym_env(args.relative_dataset_addr)

    for relative_dataset_addr in dataset_examples:

        # Setup path for labels
        example_name = os.path.splitext(os.path.basename(relative_dataset_addr))[0]
        labels_addr = os.path.join('labels', example_name)
        ...
        # Load image data
        cap = cv2.VideoCapture(relative_dataset_addr)
        frame_counter = 0

        # Initialize labeling GUI
        label_gui = KairosLabelGUI(
            dataset_addr=relative_dataset_addr,
            labels_addr=labels_addr,
            use_yolo_labels=args.use_yolo_labels)
        ...
        if args.use_yolo_labels:
            cv2.createTrackbar('Select Class:', label_gui.window_name, 0, label_gui.num_box_classes,
