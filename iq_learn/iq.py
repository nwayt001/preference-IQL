...
def iq_loss(agent, current_Q, current_v, next_v, batch):
    ...
    args = agent.args
    gamma = agent.gamma
    obs, _, action, ep_id, _, _, is_expert = batch
    is_expert = is_expert.squeeze()
    ...
    y = gamma * next_v
    reward_training_targets = (current_Q - y)
    reward = reward_training_targets[is_expert]
    ...
    elif args.method.loss != "none":
        raise ValueError(f'This sampling method is not implemented: {args.method.type}')

    if args.method.grad_pen:
        
        raise NotImplementedError('Gradient penalty has not been implemented for VPT yet.')
    ...
    return loss, loss_dict, reward_training_targets
