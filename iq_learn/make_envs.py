...
def make_env(args, monitor=True, collect_human_prefs=False):
    if 'dmc' in args.env.name:
        env = make_dcm(args)
    else:
        env = gym.make(args.env.name)

        # env wrapper that enables GUI to collect human preferences
        if collect_human_prefs:

            # TODO: importing here in case people have not updated their conda env
            from drlhp import HumanPreferencesEnvWrapper

            # make sure we have a dir to store preferences
            os.makedirs('preferences', exist_ok=True)

            env = HumanPreferencesEnvWrapper(
                env,
                args=args,
                segment_length=args.preferences.trajectory_length,
                max_prefs_in_db=10000,
                max_pref_interface_segs=10000,
                min_segments_to_test = args.preferences.min_segments_to_test,
                collect_prefs=True,
                train_reward=False,
                synthetic_prefs=False,
                log_dir='preferences',
                pref_interface_log_level=logging.INFO,
                zoom_ratio=1,
