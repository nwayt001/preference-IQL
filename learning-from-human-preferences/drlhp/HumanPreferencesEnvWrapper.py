...
# no need to split preferences in training in validation here is not
# using the reward predictor
PREFS_VAL_FRACTION = 0.0
...
def _load_or_create_pref_db(prefs_dir: str,
    ...
    if prefs_dir is not None:   
        ...
        print(f"Loaded ({n_prefs} preferences, {n_segs} segments)")
        ...
        pref_db_train = PrefDB(maxlen=n_train, args=args)
        pref_db_val = PrefDB(maxlen=n_val, args=args)
...
def _train_reward_predictor(reward_predictor_network: Callable,
    ...
    pref_buffer = _load_or_create_pref_db(prefs_dir, max_prefs, reward_predictor_logger, args)
...
class HumanPreferencesEnvWrapper(Wrapper):
    def __init__(self,
        ...
        :param args: Config file from the learning codebase.
        """
        ...
        self.preference_interface = PrefInterface(synthetic_prefs=synthetic_prefs,
                                                  ...
                                                  log_dir=log_dir,
                                                  channels=channels,
                                                  zoom=zoom_ratio, min_segments_to_test=min_segments_to_test)
        ...
        # TODO: the prefdb and save functions are tangled with reward predictor, need to split them
    ...
    def _update_episode_segment(self, obs, reward, done, action):
        """
        ...
        Also stores action, in case it is need to estimate the reward.
        ...
        self.episode_segment.append(np.copy(obs), np.copy(reward), np.copy(action))
        if done:
            while len(self.episode_segment) < self.segment_length:
                self.episode_segment.append(np.copy(obs), 0, np.copy(action))
    ...
    def step(self, action):
        ...
        if self.collecting_segments:
            self._update_episode_segment(obs, reward, done, action)
