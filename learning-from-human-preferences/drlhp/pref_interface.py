...
class PrefInterface:
    ...
    def __init__(self, synthetic_prefs, max_segs, log_dir, zoom=4, channels=3,
        ...
        # gives user teh option to end learning with a key press
    ...
    def run(self, seg_pipe, pref_pipe, kill_processes, log_level):
        ...
        while True and kill_processes.value == 0:
            ...
            if pref is not None:
                ...
                # the frames
                pref_pipe.put((s1.frames, s2.frames, pref, s1.actions, s2.actions))
            # If pref is None, the user answered "incomparable" for the segment
            # pair. The pair has been marked as tested; we just drop it.

            # start killing pref interface if user wants to end learning
            if self.end_learning:
                print("USER WANTS TO END LEARNING")
                kill_processes.value = 1
    ...
    def ask_user(self, s1, s2, log_func):
        ...
        print(f"Choose between segments {s1.hash} (LEFT_ARROW) and {s2.hash} (RIGHT_ARROW). (UP_ARROW) for equal. (DOWN_ARROW) to not comparable. Video length: {len(vid)}: ")
        while True:
            self.renderer.render(vid)
            choice = self.renderer.key_pressed
            # LEFT_ARROW = "I prefer the left segment"
            # RIGHT_ARROW = "I prefer the right segment"
            # UP_ARROW = "I don't have a clear preference between the two segments"
            # DOWN_ARROW = "I don't think the segments are comparable"
            # "" = "The segments are incomparable"
            if choice == "LEFT_ARROW" or choice == "RIGHT_ARROW" or choice == "UP_ARROW" or choice == "DOWN_ARROW" or choice == "":
                break

            # user decided to end learning, return current preference as none
            elif choice == "X":
        ...
        if choice == "LEFT_ARROW":
            ...
            pref = (0.5, 0.5)
        elif choice == "" or choice == "DOWN_ARROW":
            pref = None
        # print(f"Got preference! {pref}")
