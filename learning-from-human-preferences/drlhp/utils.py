    ...
    def imshow(self, arr):
        ...
        def on_key_press(symbol, modifier):
            ...
            if symbol == pyglet.window.key.LEFT:                
                print("Key LEFT_ARROW is pressed")
                self.key_pressed = "LEFT_ARROW"
            elif symbol == pyglet.window.key.RIGHT:                
                print("Key RIGHT_ARROW is pressed")
                self.key_pressed = "RIGHT_ARROW"
            elif symbol == pyglet.window.key.UP:                
                print("Key UP_ARROW is pressed")
                self.key_pressed = "UP_ARROW"
            elif symbol == pyglet.window.key.DOWN:                
                print("Key DOWN_ARROW is pressed")
                self.key_pressed = "DOWN_ARROW"
...
class VideoRenderer:
    ...
    def __init__(self, vid_queue, mode, fps=12, zoom=1, playback_speed=1, channels=3):
        ...
        # define valid key choices to stop renderer when user presses
        # one of these keys
        self.valid_key_choices = [
            "LEFT_ARROW",
            "RIGHT_ARROW",
            "DOWN_ARROW",
            "UP_ARROW",
    ...
    def render(self, frames):
        ...
        while True:
            ...
            self.key_pressed = v.key_pressed
            # ends video whenever user makes a preference choice (press key)
            if self.key_pressed in self.valid_key_choices:
                v.close()
                return
