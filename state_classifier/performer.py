...
from mvp.keras.layers import performer_util as util
from mvp.training.performer_kernel_transformation_types import PerformerKernelTransformationType
...
def favor_attention(query,
  ...
  """
    ...
    (None, 1, 4, 1)
    """
    # print(av_attention.shape)
    # print(attention_normalizer.shape)
    return av_attention / attention_normalizer


class Performer(tf.keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 num_heads,
                 attention_dropout,
                 kernel_transformation=PerformerKernelTransformationType.ReluKernelTransformation,
                 numerical_stabilizer=0.001,
                 causal=False,
    ...
    """
        ...
        if kernel_transformation == PerformerKernelTransformationType.ReluKernelTransformation:
            self.kernel_transformation = relu_kernel_transformation
            self.projection_matrix_type = projection_matrix_type
        elif kernel_transformation == PerformerKernelTransformationType.SoftmaxKernelTransformation:
            self.kernel_transformation = softmax_kernel_transformation
            self.projection_matrix_type = True  # Is not a type?
        else:
            raise ValueError("Kernel transformation type must be softmax or relu, got: {}".format(kernel_transformation))
    ...
    """
        ...
        if self.projection_matrix_type is None:
            ...
            seed = 0xDEADBEEF  # Does this really have to be dependent on the input??
        ...
        attention_output = favor_attention(query, key, value, self.kernel_transformation, self.causal,
                                           projection_matrix)
        """
        (None, 1, 4, 1250)
...
class SelfPerformer(Performer):
    ...
    def call(self,
             ...
             cache=None,
             decode_loop_step=None):
        return super(SelfPerformer, self).call(query_input, query_input, cache, decode_loop_step)
