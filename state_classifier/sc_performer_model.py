...
from .performer import PerformerKernelTransformationType
from .position import AddPositionalEncoding
...
class Classifier(Layer):
    def __init__(self, num_classes, activation='relu', output_activation='sigmoid', kernel_initializer='he_normal',
...
def build_state_classifier_model(num_classes, image_size=128):
    ...
    performer = Performer(768, attention_dropout=0.5, num_heads=4, kernel_transformation=PerformerKernelTransformationType.SoftmaxKernelTransformation, nb_random_features=512)
    position_encoding = AddPositionalEncoding()
    classifier = Classifier(num_classes)

    _input = Input(shape=(image_size, image_size, 3))
    x = TimeDistributed(encoder)(_input)
    x = Flatten()(x)
    x = position_encoding(x)
    x = performer(x)
    output = TimeDistributed(classifier)(x)
