...
# Using Pytorch's version of multiprocessing, as documented
# as a best practice here (uses shared memory):
...
from vpt_agent.openai_vpt.agent import resize_image, AGENT_RESOLUTION
...
def data_loader_worker(tasks_queue, output_queue, quit_workers_event, num_tasks_proc):
    """
...
class DataLoader:
    ...
    """
    def __init__(self, dataset_dir, n_workers=8, batch_size=8, n_epochs=1, max_queue_size=8, num_demos_skip = 0, demo_frac = 1):
        ...
        # unique_ids are a list of strings. Since they're not always returned in 
        # the same order, we use the following command to alphabetize them. That
        # way, when we randomize the order, we can guarantee consistency as long 
        # as the random seed doesn't change. It works fine for the random seed to be
        # set in the main function (i.e., just once for the whole program), so
        # we don't need to set the random seed here.
        unique_ids.sort()
        ...
        assert n_workers <= len(demonstration_tuples), f"n_workers ({n_workers}) should be lower or equal than number of demonstrations ({len(demonstration_tuples)})"
        ...
        if demo_frac < 1:
            last_demo = int(demo_frac * len(self.demonstration_tuples))
            self.demonstration_tuples = self.demonstration_tuples[:last_demo]

        # If num_demos_skip > 0, we skip the first num_demos_skip demo tuples:
        self.demonstration_tuples = self.demonstration_tuples[num_demos_skip:]
        
        self.total_tasks = len(self.demonstration_tuples)

        # To keep track of how many tasks are either completed or currently being processed:
        self.num_tasks_proc = Value('i', 0)
    ...
    def get_specific_next_outputs(self, idxs):
        """
        Added this function to get experience tuples from specific workers,
        rather than needing to cycle through all of them in a fixed order.
        """        
        batch_frames = []
        batch_actions = []
        batch_episode_id = []
        ...
        for idx in idxs:
            ...
            workitem = self.output_queues[idx].get(timeout=QUEUE_TIMEOUT)
    ...
    def get_progress_info(self):
        ...
        return self.num_tasks_proc.value, self.total_tasks
