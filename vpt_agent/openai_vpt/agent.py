...
from vpt_agent.openai_vpt.lib.action_mapping import CameraHierarchicalMapping
from vpt_agent.openai_vpt.lib.actions import ActionTransformer
from vpt_agent.openai_vpt.lib.policy import MinecraftAgentPolicy, MinecraftAgentPolicyQ
from vpt_agent.openai_vpt.lib.torch_util import default_device_type, set_default_torch_device
...
class MineRLAgent:
    def __init__(self, args = {}, device=None, policy_kwargs=None, pi_head_kwargs=None,
                 Q_head = False, reward_head = False):
        self.Q_head = Q_head
        if device is None:
        ...
        else:   # If Q_head, then set up Q version of policy.

            self.action_dim = ACTION_DIM

            agent_kwargs['action_dim'] = ACTION_DIM
            agent_kwargs['agent'] = self
            agent_kwargs['action_emb_frac'] = args.action_emb_frac
            self.policy = MinecraftAgentPolicyQ(**agent_kwargs, reward_head = reward_head).to(device)
            
        self.hidden_state = self.policy.initial_state(1)
        self._dummy_first = th.from_numpy(np.array((False,))).to(device)

    def load_weights(self, path, weights_state_dict = None):
        """Load model weights from a path, and reset hidden state"""
        if weights_state_dict is None:
            self.policy.load_state_dict(th.load(path, map_location=self.device), strict=False)
        else:
            self.policy.load_state_dict(weights_state_dict, strict = False)
    ...
    def _env_obs_to_agent(self, minerl_obs, numpy = False):
        """
    ...
    def get_action(self, minerl_obs):
        ...
        if not self.Q_head:
            ...
            # more things to return now with the Q head
            agent_action, self.hidden_state, _, _, _, _ = self.policy.act(
                agent_input, self._dummy_first, self.hidden_state,
                stochastic=True
            )
