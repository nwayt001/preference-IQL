...
from vpt_agent.openai_vpt.lib.action_head import make_action_head
from vpt_agent.openai_vpt.lib.impala_cnn import ImpalaCNN
from vpt_agent.openai_vpt.lib.scaled_mse_head import ScaledMSEHead
from vpt_agent.openai_vpt.lib.Q_head import QHead
from vpt_agent.openai_vpt.lib.reward_head import RewardHead
from vpt_agent.openai_vpt.lib.tree_util import tree_map
from vpt_agent.openai_vpt.lib.util import FanInInitReLULayer, ResidualRecurrentBlocks

from vpt_agent.openai_vpt.lib.action_embedding import ActionEmbedding
...
class MinecraftPolicy(nn.Module):
    ...
    def forward(self, ob, state_in, context):
        ...
        # TODO: incosistently using obs as dicts or tensors
        try:
            # used before learning steps
            x = self.img_preprocess(ob["img"])
        except:
            # used afer learning begins
...
class MinecraftAgentPolicyQ(nn.Module):
    def __init__(self, action_space, agent, action_dim: int, action_emb_frac: int,
                 policy_kwargs, pi_head_kwargs, reward_head = False):
        """
        action_emb_size: dimension of either the action embedding
        that will be given to the Q-network to calcualate action-values.
        """
        super().__init__()
        self.net = MinecraftPolicy(**policy_kwargs)

        self.output_latent_size = self.net.output_latent_size()

        self.action_space = action_space
        
        # Create action embedding:
        num_tiles = int(np.round(self.output_latent_size * action_emb_frac / action_dim))
        self.act_embedding = ActionEmbedding(agent, action_dim, num_tiles = num_tiles)
        
        self.action_emb_size = self.act_embedding.embedding_dim

        self.Q_head = self.make_Q_head(self.output_latent_size, self.action_emb_size)
        self.pi_head = self.make_action_head(self.output_latent_size, **pi_head_kwargs)
        
        if reward_head:
            self.reward_head = self.make_reward_head(self.output_latent_size, self.action_emb_size)
        self.use_reward = reward_head
    ...
    def make_Q_head(self, v_out_size: int, action_size: int, 
                    norm_type: str = "ewma", norm_kwargs: Optional[Dict] = None):
        return QHead(v_out_size, 1, action_size, norm_type=norm_type, norm_kwargs=norm_kwargs)

    def make_reward_head(self, v_out_size: int, action_size: int, 
                    norm_type: str = "ewma", norm_kwargs: Optional[Dict] = None):
        return RewardHead(v_out_size, 1, action_size, norm_type=norm_type, norm_kwargs=norm_kwargs)
    ...
    def reset_parameters(self):
        ...
        #self.net.reset_parameters()
        #self.pi_head.reset_parameters()
        self.Q_head.reset_parameters()
        
        if self.use_reward:
            self.reward_head.reset_parameters()

    def forward(self, obs, first: th.Tensor, state_in, action = None, eval_policy_head = True):
        """
        Q-value is only calculated if action is not None.
        action is assumed to be in the correct format for passing to the Q-head.
        """
        if isinstance(obs, dict):
            # We don't want to mutate the obs input.
            obs = obs.copy()
        ...
        (pi_h, q_h), state_out = self.net(obs, state_in, context={"first": first})
        ...
        if action is not None:
            Qpred = self.Q_head(q_h, action).squeeze(2)
        else:
            Qpred = None

        return (pi_logits, Qpred, None), state_out, pi_h, q_h
    ...
    def get_output_for_observation(self, obs, state_in, first, action = None):
        """
        Return gradient-enabled outputs for given observation.
        ...
        (pd, Qpred, _), state_out, pi_h, q_h = self(obs=obs, first=first, state_in=state_in,
                                         action = action)

        return pd, self.Q_head.denormalize(Qpred)[:, 0], state_out, pi_h, q_h


    def sample(self, obs, first, state_in, stochastic: bool = True):
        (pd, Qpred, _), state_out, pi_h, q_h = self(obs=obs, first=first, state_in=state_in)

        ac = self.pi_head.sample(pd, deterministic=not stochastic)

        # Get embedded action:
        ac_emb = self.act_embedding.policy_action_to_embedding(ac).to(self.device)

        # Get Q-value associated with the selected action:
        Qpred = self.Q_head(q_h, ac_emb).squeeze(2)
    
        log_prob = self.pi_head.logprob(ac, pd)
        assert not th.isnan(log_prob).any()

        return ac, state_out, log_prob, Qpred, pi_h, q_h, ac_emb, pd
    ...
    def act(self, obs, first, state_in, stochastic: bool = True, taken_action=None, return_pd=False):
        ...
        if taken_action is None:
            ...
            # add fictitious time dimension to match rest of the code
            #ac_emb = tree_map(lambda x: x.unsqueeze(0), ac_emb)
    
            # Get Q-value associated with the selected action:
            Qpred = self.Q_head(q_h, ac_emb)
        ...
        # After unsqueezing, squeeze back to remove fictitious time dimension
        result = {"log_prob": log_prob[:, 0], "Qpred_denorm": self.Q_head.denormalize(Qpred)[:, 0],
                  "Q_pred": Qpred[:, 0]}
        if return_pd:
            result["pd"] = tree_map(lambda x: x[:, 0], pd)
        ac = tree_map(lambda x: x[:, 0], ac)
        ac_emb = tree_map(lambda x: x[:, 0], ac_emb)

        return ac, state_out, result, pi_h, q_h, ac_emb

    @th.no_grad()
    def q(self, obs, first, state_in, action):
        """Predict value for a given mdp observation and action"""
        obs = tree_map(lambda x: x.unsqueeze(1), obs)
        first = first.unsqueeze(1)

        (pd, Qpred, _), state_out, _, _ = self(obs=obs, first=first, state_in=state_in,
                                         action = action)

        # After unsqueezing, squeeze back
        return self.Q_head.denormalize(Qpred)[:, 0]
